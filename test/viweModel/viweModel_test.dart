...
void main() {
 group("test viwe_model", () {
   ...
   blocTest<Viwe_Model, Viwe_State>(
     'test sign up',
     build: () {
       when(SignUp_UsaCase.signUp(any)).thenAnswer((_) async {});
       return Viwe_Model(
         SignUp_UsaCase,
         SignInUseCase,
         Get_subject_Use_Case,
   ...
   blocTest<Viwe_Model, Viwe_State>(
     'tast_sign_in',
     build: () {
       when(SignInUseCase.signIn(any)).thenAnswer((_) async => Sign_in_response(
         message: "test",
         token: "test",
       ));
       return Viwe_Model(
   ...
   blocTest<Viwe_Model, Viwe_State>(
     "Get_subject_Use_Case",
         build: () {
           when(Get_subject_Use_Case.Get_subjects_Data(any)).thenAnswer((_) async => [
     ...
     expect: () => [
       Viwe_State.initial().copyWith(isLoading: true),
       Viwe_State.initial().copyWith(isLoading: false, sucsses: "sucsses"),
     ],
     verify: (bloc) {
   ...
   blocTest<Viwe_Model, Viwe_State>(
     "ForgotPasswordUseCase",
         build: () {
           when(ForgotPasswordUseCase.forgotPasswordSendCode(any)).thenAnswer((_) async {});
           return Viwe_Model(
             SignUp_UsaCase,
             SignInUseCase,
             Get_subject_Use_Case,
   ...
   blocTest<Viwe_Model, Viwe_State>(
     "VerifyResetCodeUseCase",
     build: () {
       when(VerifyResetCodeUseCase.verifyResetCode(any)).thenAnswer((_) async {});
       return Viwe_Model(
         SignUp_UsaCase,
         SignInUseCase,
         Get_subject_Use_Case,
   ...
   blocTest<Viwe_Model, Viwe_State>(
     "ResetPasswordUseCase",
     build: () {
       when(ResetPasswordUseCase.resetPassword(any)).thenAnswer((_) async {});
       return Viwe_Model(
         SignUp_UsaCase,
         SignInUseCase,
         Get_subject_Use_Case,
